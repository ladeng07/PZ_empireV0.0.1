...
class Button(pygame.Surface):
    def __init__(self,sf,size,position,text=None,c=1):
        self.sf=sf
        self.size=size
        self.position=position
        self.status=0
...
def choice(w,h):
    lh=h//12
    return [Button(button,(w//6,h//12),((w-w//2)//2+w//24,(h-int(h//1.5))//2+h//16),['初级试卷:导学案','学习力+5 魅力-10','执行力-10 行动力-20'],c=0),
            Button(button,(w//6,h//12),((w-w//2)//2+w//24,(h-int(h//1.5))//2+h//16+h//12),['初级试卷:导学案','学习力+5 魅力-10','执行力-10 行动力-20'],c=0)]
    ...
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONUP:
            ...
            if study_b.get_range()[0] < mouse_pos[0] < study_b.get_range()[2] and study_b.get_range()[1] < mouse_pos[1] < study_b.get_range()[3] and start_game == 1 and study_b.status:
                charm += 1
                b=Label(mes_box,(w//2,int(h//1.5)),((w-w//2)//2,(h-int(h//1.5))//2))
                l=[Label(top_label,(int(w//5),int(h//2.5)),(0,0)),
                   ...
                   labelbox('行动力   %d'% energy,(0,int(lh*4.5))),
