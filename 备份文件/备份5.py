...
while 1:
    ...
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONUP:
            ...
            if shop_b.status and study_b.status and mouse_pos[0] > 0 and mouse_pos[1] > h-h//4 and 19 < rd< 100:
                flush(l)
                print(l[-1])
                shop_b.status=0

            if  study_b.status and shop_b.get_range()[0] < mouse_pos[0] < shop_b.get_range()[2] and shop_b.get_range()[1] < mouse_pos[1] < shop_b.get_range()[3] and start_game == 1 and 19< rd < 100:
                t='商店功能功能暂未开放，敬请期待'
            ...
            if  study_b.status and social_b.status and mouse_pos[0] > 0 and mouse_pos[1] > h-h//4 and 19 < rd< 100:
                flush(l)
                print(l[-1])
                print('ile')
                social_b.status=0
            ...
            if  study_b.status and social_b.get_range()[0] < mouse_pos[0] < social_b.get_range()[2] and social_b.get_range()[1] < mouse_pos[1] < social_b.get_range()[3] and start_game == 1 and 19< rd < 100:
                t='社交功能暂未开放，敬请期待'
                print('dierci')
                textlabel(t,(w//7.5,h-h//5.5),[Label(text_box,(w//5*4,h//5),(w//10,h-h//5))],(w//1.4,h//1.2),0,0)
                social_b.status=1
            ...
            if  not social_b.status and not shop_b.status and study_b.c and not study_b.status and start_game and rod>0:
                if not b.get_rect()[0]<mouse_pos[0] or not b.get_rect()[1] < mouse_pos[1] < b.get_rect()[1]+b.get_rect()[3]:
                    if not social_b.get_range()[0] < mouse_pos[0] < social_b.get_range()[2] and not social_b.get_range()[1] < mouse_pos[1] < social_b.get_range()[3]:
                        print('好的')
            ...
            if event.button == 1 and start_game==0 and start_b.get_range()[0] < mouse_pos[0] <start_b.get_range()[2] and start_b.get_range()[1] < mouse_pos[1] < start_b.get_range()[3] and rd==0:
                ...
                l=[Label(start_bg,(w,h),(0,0)),Label(text_box,(w//5*4,h//5),(w//10,h-h//5))]
                start_g(l,size)
                textlabel('ggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg欢迎来到浦中帝国！！本游戏没有一丝丝教程，大家伙凑合这玩吧哈哈哈哈哈哈哈哈哈',(w//7.5,h-h//5.5),l,(w//1.4,h//1.2),0,0)
                rd += 1
