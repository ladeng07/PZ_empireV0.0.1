...
e_button=pygame.transform.smoothscale(pygame.image.load(e_button).convert_alpha(),(64,64))
...
def big_event():
    ...
    for i in range(len(el)):
        screen.blit(e_button,(w//5+i*3*e_button.get_rect()[3]//2,int(h-e_button.get_rect()[3]*1.5)))
        screen.blit(t,(w//5+i*3*e_button.get_rect()[3]//2+(e_button.get_rect()[2]-t.get_rect()[2])//2,int(h-e_button.get_rect()[3]//2)))

def c_label(l,result=False):
    ...
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONUP:
            ...
            if event.button == 1 and start_game==1 and len(el) != [] and event_num == 0:
                if w//5 < mouse_pos[0] < w//5+e_button.get_rect()[2] and int(h-e_button.get_rect()[3]*1.5) < mouse_pos[1] < h-e_button.get_rect()[3]//2:
                    print(1)
                    event_num=1
                elif w//5 +3*e_button.get_rect()[3]//2 < mouse_pos[0] < w//5 +5*e_button.get_rect()[3]//2 and int(h-e_button.get_rect()[3]*1.5) < mouse_pos[1] < h-e_button.get_rect()[3]//2 and len(el) > 1:
                    print(2)
                elif w//5 +3*e_button.get_rect()[3] < mouse_pos[0] < w//5 +4*e_button.get_rect()[3] and int(h-e_button.get_rect()[3]*1.5) < mouse_pos[1] < h-e_button.get_rect()[3]//2 and len(el) > 2:
                    print(3)
