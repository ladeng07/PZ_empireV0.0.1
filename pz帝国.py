...
start_bg4='Background/start_bg4.jpeg'
...
skip='Label/skip.jpg'
#goroom='Label/goroom.png'
e_button='Label/event_button.jpg'
choice_button='Label/choice_button.jpg'
p1='Label/p1.jpg'
s1='Label/s1.jpeg'
#s2='s2.jpeg'
#s3='s3.jpeg'
...
start_bg4=pygame.image.load(start_bg4).convert()
...
e_button=pygame.transform.smoothscale(pygame.image.load(e_button).convert_alpha(),(50,50))
choice_button=pygame.image.load(choice_button).convert_alpha()
...
def big_event():
    f=pygame.font.SysFont('华文中宋',size=16)
    t=f.render(' 事 件 ',True,(0,0,0),(255,255,255))
    for i in range(len(el)):
        screen.blit(e_button,(w//10+i*3*e_button.get_rect()[3]//2,int(h-e_button.get_rect()[3]*1.5)))
        screen.blit(t,(w//10+i*3*e_button.get_rect()[3]//2+(e_button.get_rect()[2]-t.get_rect()[2])//2,int(h-e_button.get_rect()[3]//2)))

def c_label(el,result=False):
    c=pygame.transform.smoothscale(choice_button,(w//3,h//8))
    screen.blit(c,((w-c.get_rect()[2])//2,(h-c.get_rect()[3])//2))
    screen.blit(c,((w-c.get_rect()[2])//2,(h-c.get_rect()[3])//2+c.get_rect()[3]))
    screen.blit(c,((w-c.get_rect()[2])//2,(h-c.get_rect()[3])//2-c.get_rect()[3]))
    l=[Label(text_box,(w//5*4,h//5),(w//10,h-h//5))]
    if result:
        t=el[4]
...
while 1:
    ...
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONUP:
            ...
            if event.button == 1 and start_game==1 and len(el) != [] and event_num == 0:
                if w//10 < mouse_pos[0] < w//10+e_button.get_rect()[2] and int(h-e_button.get_rect()[3]*1.5) < mouse_pos[1] < h-e_button.get_rect()[3]//2:
                    print(1)
                    event_num=1
                    c_label(el)
                elif w//10 +3*e_button.get_rect()[3]//2 < mouse_pos[0] < w//10 +5*e_button.get_rect()[3]//2 and int(h-e_button.get_rect()[3]*1.5) < mouse_pos[1] < h-e_button.get_rect()[3]//2 and len(el) > 1:
                    print(2)
                elif w//10 +3*e_button.get_rect()[3] < mouse_pos[0] < w//10 +4*e_button.get_rect()[3] and int(h-e_button.get_rect()[3]*1.5) < mouse_pos[1] < h-e_button.get_rect()[3]//2 and len(el) > 2:
                    print(3)

            if event_num < 0:
                if event_num == -1:
                    event_num=0
                    c_label(el,result=True)
            ...
            if rd>79 and shop_b.status and  shop_b.c and shop_b.get_range()[0] < mouse_pos[0] < shop_b.get_range()[2] and shop_b.get_range()[1] < mouse_pos[1] < shop_b.get_range()[3]:#关闭小黑按钮
                shop_b.status=0
                shop_b.c=1
                
                
            if rd>79 and shop_b.status and  not shop_b.c and shop_b.get_range()[0] < mouse_pos[0] < shop_b.get_range()[2] and shop_b.get_range()[1] < mouse_pos[1] < shop_b.get_range()[3]:#打开小黑按钮
                start_bg4=pygame.transform.smoothscale(start_bg4,size)
                screen.blit(start_bg4,(0,0))
                screen.blit(pygame.transform.smoothscale(goroom,shop_b.get_rect()[2:]),shop_b.get_rect()[:2])
                shop_b.c=1
                shop_status=1
                study_b.status=0
                social_b.status=0
            ...
            if rd>79 and not shop_b.status and shop_b.c and start_game==1:                        #关闭小黑按钮后的刷新
            ...
            if social_b.c and social_b.status and start_game and rod>0:
                if  b.get_rect()[0] > mouse_pos[0] or b.get_rect()[1] > mouse_pos[1]  or mouse_pos[1] >  b.get_rect()[1]+b.get_rect()[3]:   #选择社交内容或关闭
            ...
            if event_num < 0:
                if event_num == -1:
                    c_label(el[-event_num-1],result=True)
                    event_num=0
                    start_game=-3

            if event_num > 0:
                if event_num == 1:
                    c_label(el[event_num-1])
                    event_num = -event_num

                
            if event.button == 1 and rd==78 and start_game == 1:
                pygame.event.set_blocked([MOUSEBUTTONUP])
                t='(小黑现已开放，快去看看有什么好东西吧)'
                textlabel(t,(w//7.5,h-h//5.5),l,(w//1.4,h//1.2),0,0)
                start_game=-3
                rd += 1
